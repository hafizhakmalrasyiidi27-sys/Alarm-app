...
if len(st.session_state.alarms) == 0:
    ...
    for idx in range(len(st.session_state.alarms)):
        ...
        with col2:
            st.markdown(", ".join(alarm.repeat) if alarm.repeat else "No Repeat")
        with col3:
...
if st.session_state.triggered:
    ...
    trig_data = st.session_state.triggered
    st.warning(f"⏰ Alarm: **{trig_data['label']}** is ringing!")

    # SOUND
    st.markdown(
        f"""
        <audio id="alarm_audio" autoplay loop>
            <source src="{trig_data['sound_url']}" type="audio/ogg">
        </audio>
        """,
        unsafe_allow_html=True
    )
    ...
    if st.button("Stop Alarm"):
        ...
        # SAVE LOG
        label = trig_data.get("label", "Alarm")
        start_time = trig_data.get("start_time", time.time())
        duration = int(time.time() - start_time)

        st.session_state.logs.append(AlarmLog(label, duration))
